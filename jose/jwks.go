...
func (s *JSONWebKeySet) FindForSigning(alg string) *JSONWebKey {
	return s.find(alg, UseSig)
}

// FindForEncryption returns the JSONWebKey intended for encryption use with the matching algorithm, or nil if not found.
func (s *JSONWebKeySet) FindForEncryption(alg string) *JSONWebKey {
	return s.find(alg, UseEnc)
}
