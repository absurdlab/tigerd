...
func TestEncode(t *testing.T) {
	...
	for _, c := range []struct {
		...
		},
		{
			...
			claims: new(testStdClaims).init(),
			opt: []EncoderOpt{
				WithEncryption(RSA_OAEP_256, A128GCM, jwks),
			},
			assert: func(t *testing.T, token string) {
				parsed, err := jwt.ParseEncrypted(token)
				if assert.NoError(t, err) {
					claims := new(testStdClaims)
					_ = parsed.Claims(jwks.FindForEncryption(RSA_OAEP_256).Key, &claims)
					claims.assert(t)
				}
			},
		},
		{
			...
			claims: new(testStdClaims).init(),
			opt: []EncoderOpt{
				WithSignature(ES256, jwks),
				WithEncryption(RSA_OAEP_256, A128GCM, jwks),
			},
			assert: func(t *testing.T, token string) {
				parsed, err := jwt.ParseSignedAndEncrypted(token)
				if assert.NoError(t, err) {
					parsed2, err := parsed.Decrypt(jwks.FindForEncryption(RSA_OAEP_256).Key)
					if assert.NoError(t, err) {
						claims := new(testStdClaims)
