...
func WithSignature(alg string, jwks *JSONWebKeySet) EncoderOpt {
	return func(n *encoder) error {
		if IsNoneOrEmpty(alg) {
			return nil
		}
...
func WithEncryption(alg string, enc string, jwks *JSONWebKeySet) EncoderOpt {
	return func(n *encoder) error {
		if IsNoneOrEmpty(alg) || IsNoneOrEmpty(enc) {
			return nil
		}
...
func (n *encoder) createEncrypter(signed bool) (jose.Encrypter, error) {
	if n.encryptionKey == nil || IsNoneOrEmpty(n.encryptionEnc) {
		return nil, nil
	}
	...
	encrypter, err := jose.NewEncrypter(jose.ContentEncryption(n.encryptionEnc), rcpt, opts)
	if err != nil {
		return nil, err
	}
